BEGIN;
        INSERT INTO "user"
        ("id", "name", "email", "password", "city", "picture")
        VALUES
        ('1', 'John', 'John@oclock.io', 'john', 'Paris', 'https://thumbs.dreamstime.com/b/vecteur-d-ic%C3%B4ne-de-profil-avatar-par-d%C3%A9faut-image-sociale-utilisateur-m%C3%A9dias-social-182145777.jpg');
        
        INSERT INTO "user"
        ("id", "name", "email", "password", "city", "picture")
        VALUES
        ('2', 'Donneur', 'Donneur@oclock.io', 'Donneur', 'Creteil', 'https://thumbs.dreamstime.com/b/vecteur-d-ic%C3%B4ne-de-profil-avatar-par-d%C3%A9faut-image-sociale-utilisateur-m%C3%A9dias-social-182145777.jpg');
        
        INSERT INTO "work"
        ("id", "title", "resume", "category_id")
        VALUES
        ('1', 'Alice aux pays des merveilles', 'loremdnkjndlqskdjqlsjdqklzope o ,dqod ,qd qzpd,zq,dzqpokdqlmkdzqmldqpzq', '4');
        
        INSERT INTO "work"
        ("id", "title", "resume", "category_id")
        VALUES
        ('2', 'Harry Potter et l’école des sorciers', 'loremdnkjndlqskdjqlsjdqklzope o ,dqod ,qd qzpd,zq,dzqpokdqlmkdzqmldqpzq', '1');
        
        INSERT INTO "author"
        ("id", "lastname", "firstname")
        VALUES
        ('1', 'Lewis', 'Carroll');
        
        INSERT INTO "author"
        ("id", "lastname", "firstname")
        VALUES
        ('2', 'JK', 'Rowling');
        
        INSERT INTO "category"
        ("id", "name")
        VALUES
        ('1', 'Fantastique');
        
        INSERT INTO "category"
        ("id", "name")
        VALUES
        ('2', 'Fiction absurde');
        
        INSERT INTO "category"
        ("id", "name")
        VALUES
        ('3', 'Fantasy littéraire');
        
        INSERT INTO "category"
        ("id", "name")
        VALUES
        ('4', 'Littérature pour enfants');
        
        INSERT INTO "book"
        ("id", "isbn_13", "cover_page", "editor", "publication_date", "language", "pages_number", "height", "width", "thickness", "work_id")
        VALUES
        ('1', '9782215126423', 'https://m.media-amazon.com/images/W/IMAGERENDERING_521856-T1/images/I/61fNOqZlsRL._SX218_BO1,204,203,200_QL40_ML2_.jpg', 'Fleurus', '24 octobre 2013', 'Français', '176', '123', '456', '25', '1');
        
        INSERT INTO "book"
        ("id", "isbn_13", "cover_page", "editor", "publication_date", "language", "pages_number", "height", "width", "thickness", "work_id")
        VALUES
        ('2', '9782215126548', 'data:image/jpeg;base64,', 'Les sorcier', '10 octobre 2004', 'Français', '140', '10', '15', '8', '2');
        
        INSERT INTO "author_has_work"
        ("id", "work_id", "author_id")
        VALUES
        ('1', '1', '1');
        
        INSERT INTO "author_has_work"
        ("id", "work_id", "author_id")
        VALUES
        ('2', '2', '2');
        
        INSERT INTO "user_has_book"
        ("id", "book_id", "user_id","disponibility", "status")
        VALUES
        ('1', '1', '2', 'disponible', 'bon état');
        
        INSERT INTO "user_has_book"
        ("id", "book_id", "user_id","disponibility", "status")
        VALUES
        ('2', '2', '2', 'donné', 'mauvais');
        COMMIT;